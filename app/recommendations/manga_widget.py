...
class TableLayout(QVBoxLayout):
    ...
    def __init__(self, database, number):
        ...
        self.switch = QPushButton('Switch to Anime.')
        ...
        self.page_selector = PageSelector((number // 10) + 1)
...
class MangaWidget(QWidget):
    ...
    def __init__(self, database, count):
        super(MangaWidget, self).__init__()
        ...
        self.recommendation_table.switch_page.connect(lambda: self.switch.emit(0))
    ...
    def table_clicked(self, data):
        manga, genres, authors = data
        self.info_widget.change_info(*manga, genres, authors, False)
