...
class ResultsTable(QTableWidget):
    ...
    def anime(self, data):
        ...
        self.setItem(row, 3, QTableWidgetItem(f'Episodes: {episodes if episodes else "?"}'))
    ...
    def manga(self, data):
        ...
        self.setItem(row, 1, title)
        self.setItem(row, 2, QTableWidgetItem(f'Type: {type_}'))
        self.setItem(row, 3, QTableWidgetItem(f'Volumes: {volumes if volumes else "?"}'))
        if chapters is not None:
            self.setItem(row, 4, QTableWidgetItem(f'Chapters: {chapters if volumes else "?"}'))
        self.setItemDelegateForRow(row, ReadOnlyDelegate(self))
        self.resizeColumnsToContents()
        self.resizeRowsToContents()
